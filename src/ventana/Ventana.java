...
public class Ventana {
	...
	public void UpdateApi() {
	    new Thread(new Runnable() {
	        ...
	        public void run() {
	            ...
	            ArrayList<String> listasIpsAux = new ArrayList<String>();
	            for (int i = 1; i < MulticastControl.getMyIps().size(); i++) {
	                listasIpsAux.add(MulticastControl.getMyIps().get(i));
	            }
	            if (MulticastControl.ip_servidor != null && !MulticastControl.ip_servidor.isEmpty()) {
	                if (!listasIpsAux.contains(MulticastControl.ip_servidor)) {
	                    listasIpsAux.add(MulticastControl.ip_servidor);
	                }
	            }
	            JsonData j = new JsonData();
	            j.setIps(listasIpsAux);
	            if (listasIpsAux.size() != 0) {
	                try {
	                    TokenClepnid response = Controlador.sendPostRequestVer(1, token);

	                    if (response == null) {
	                        Controlador.sendPostRequestCrear(1, token, j, new Date());
	                    } else {
	                        if (Controlador.isUpdated(listasIpsAux, response.getJsonData().getIps())) {
	                            Controlador.sendPostRequestUpdate(1, response.getId(), token, j, new Date());
	                        }
	                    }
	                } catch (IOException e) {
	                    e.printStackTrace();
	...
	public boolean mostrarContenidoPorPantalla(Contenido contenido) {
		if (contenido != null) {
			if (this.contenido == null || !(contenido == null)) {
				UpdateApi();
				Actividad actividad;
				multicastControl.cliente.cambioServidor();
				boolean soyServidor = multicastControl.soyServidor;
				...
				case Html:
					...
					if (soyServidor) {
						actividad = ActividadApi.crearActividad(0, "Red local este equipo", Configuracion.getNombreRed());
					}else {
						actividad = ActividadApi.crearActividad(0, "Red local otro equipo", Configuracion.getNombreRed());
					}
					actividad.anyadirALista("Introducido en el sistema elemento Html");

					CompletableFuture.runAsync(() -> {ActividadApi.getInstance().addActividadExterna(actividad);});
				...
				case Texto:
					...
					actividad.anyadirALista("Introducido en el sistema elemento Texto");
					CompletableFuture.runAsync(() -> {ActividadApi.getInstance().addActividadExterna(actividad);});
					this.contenido = contenido;
					return true;
				case Ficheros:
					...
					actividad.anyadirALista("Introducida lista de ficheros en el sistema");
					for (int i = 0; i < contenido.listaFicheros.size(); i++) {
						...
						actividad.anyadirALista(contenido.listaFicheros.get(i)[0]+", "+contenido.listaFicheros.get(i)[1]);
					}
					CompletableFuture.runAsync(() -> {ActividadApi.getInstance().addActividadExterna(actividad);});
					this.contenido = contenido;
					return true;
	...
	public void open() {
		...
		token = "";
		try {
			Configuracion c = Configuracion.deserializar();
			token = c.codigoApp;
