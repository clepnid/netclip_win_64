...
import ventanaCodigoApp.VentanaCodigoApp;
import ventanaGestionarModulo.VentanaGestionarModulosWeb;

public class MenuBar {
	private Ventana ventana;
	private Boolean ventanaEliminarRutaAbierta, ventanaAyudaAbierta, ventanaObtenerLinkAbierta, ventanaCodigoAppAbierta;
	private ventanaCodigoApp.VentanaCodigoApp ventanaCodigoApp;

	public MenuBar(Ventana ventana) {
		this.setVentana(ventana);
		ventanaEliminarRutaAbierta = false;
		ventanaCodigoAppAbierta = false;
		...
		// si no se ha cargado la configuracion del menu no se podr√° realizar esta
		// accion
		...
		mostrarVentanaGestionModulo.setText("CÛdigo escaner app");
		mostrarVentanaGestionModulo.addListener(SWT.Selection, new Listener() {

			@Override
			public void handleEvent(Event event) {
				if (!ventanaCodigoAppAbierta) {
					Shell shell = new Shell(ventana.shlSwt);
					shell.setText("CÛdigo App");
					shell.setLayout(new GridLayout(1, false));
					...
					String codigo;
					try {
						Configuracion aux = Configuracion.deserializar();
						codigo = aux.codigoApp;
					...
					// Crear el widget StyledText para mostrar el cÛdigo sin barras de
					// desplazamiento
					StyledText styledText = new StyledText(shell, SWT.BORDER);
					styledText.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, true, true)); // Centrado
					styledText.setText(codigo); // Insertar el cÛdigo

					// Establecer fondo blanco para el StyledText
					shell.setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));

					// Establecer la fuente m·s grande
					FontData fontData = new FontData("Arial", 48, SWT.BOLD); // Cambiar el tamaÒo y tipo de fuente
					Font font = new Font(shell.getDisplay(), fontData);
					styledText.setFont(font);

					// Asegurarse de que el widget pueda recibir el foco
					styledText.setFocus(); // Forzar que sea focusable y estÈ enfocado

					styledText.setEditable(false); // Solo lectura

					// Crear un botÛn para cambiar el texto
					...
					// AcciÛn para el botÛn de cambiar
					cambiarButton.addListener(SWT.Selection, new Listener() {

						public void handleEvent(Event event) {
							String nuevoCodigo = Configuracion.generateRandomStrings(10);
							Configuracion aux;
							try {
								aux = Configuracion.deserializar();
								aux.codigoApp = nuevoCodigo;
								Configuracion.serializar(aux);
								styledText.setText(nuevoCodigo);
							} catch (ClassNotFoundException | IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
					...
					// Establecer el tamaÒo de la ventana
		...
		menuAyuda.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				if (!ventanaAyudaAbierta) {
					...
					label7.setText("si hay compartido archivos: se nos abrir√° una ventana");
					...
					label9.setText("texto con estilos se pegar√° donde seleccionemos anteriormente.");
	...
	public boolean isventanaCodigoAppAbierta() {
		return ventanaCodigoAppAbierta;
	}

	public void setVentanaGestionModuloAbierta(boolean bool) {
		this.ventanaCodigoAppAbierta = bool;
	}

	public VentanaCodigoApp getVentanaCodigoApp() {
		return ventanaCodigoApp;
	}

	public void setVentanaCodigoApp(VentanaCodigoApp ventanaCodigoApp) {
		this.ventanaCodigoApp = ventanaCodigoApp;
	}
