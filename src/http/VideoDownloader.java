...
public class VideoDownloader extends Thread {
	...
	public void run() {
		if (Ventana.OS.equals("WINDOWS")) {
			...
			String[] commands = getComandosWindows();
			Process proc;
			BufferedReader stdInput = null;
			BufferedReader stdError = null;
			try {
				processBuilder.directory(new File(System.getProperty("user.dir") + File.separator + "src"
						+ File.separator + "html" + File.separator + "modulo_youtube_downloader"));
				...
				if (fileToUp != null) {
					...
					if (!yaIntroducido && JsonModulosMenuWeb.config != null) {
						...
						ArrayList<ConfiguracionJson> listaModulos = JsonModulosFicheros
								.obtenerConfiguraciones(extension);
						JsonEntradaMenuModulo modulo = new JsonEntradaMenuModulo();
						if (listaModulos != null) {
							for (ConfiguracionJson configuracionJson : listaModulos) {
								Ventana.getInstance().http.crearUrlModulo(configuracionJson, nombre,
										Configuracion.deserializar().carpeta + File.separator + fileToUp);
						...
						Actividad actividad = ActividadApi.crearActividad(0, "VideoDownloader", Configuracion.getNombreRed());
						actividad.anyadirALista("Introducido en el sistema: "+nombre);
			    ...
			    String commands = getComandosLinux(); 
	...
	private String[] getComandosWindows() {
		String carpeta;
		try {
		...
		switch (this.tipo) {
		case YoutubeAudio:
			String[] commands = { "./src/html/modulo_youtube_downloader/yt-dlp.exe", "-o",
					carpeta + File.separator + this.outputName.replace(" ", "%20") + ".%(ext)s", "--extract-audio",
					"--audio-format", "mp3", this.url };
			fileToUp = this.outputName + ".mp3";
			return commands;

		case YoutubeVideo:
			String[] commands1 = { "./src/html/modulo_youtube_downloader/ffmpeg-win.exe", "--url", this.url, "-f",
					this.format, "-q", this.quality, "-o", carpeta + File.separator + this.outputName };
			fileToUp = this.outputName + "." + this.format;
			return commands1;

		case OtherAudio:
			String[] commands2 = { "./src/html/modulo_youtube_downloader/yt-dlp.exe", "-o",
					carpeta + File.separator + this.outputName.replace(" ", "%20") + ".%(ext)s", "--extract-audio",
					"--audio-format", "mp3", this.url };
			fileToUp = this.outputName + ".mp3";
			return commands2;
		...
		case OtherVideo:
			String[] commands3 = { "./src/html/modulo_youtube_downloader/yt-dlp.exe", "-o",
					carpeta + File.separator + this.outputName.replace(" ", "%20"), this.url };
	...
	private String getComandosLinux() {
	    String carpeta;
	    try {
	        carpeta = Configuracion.deserializar().carpeta;
	        carpeta.replace(" ", "%20");
